---
description: cosmo /duration — [value] | extend <value> — view/set the fuzz duration cap (capped by config)
allowed-tools: Bash(cosmo:*), Bash(python -m cosmo:*)
---

Run the cosmo `/duration` command in a live session over the current target and report the result.
...
1. Open a session on the target (current working tree, or a PR the user named) — opening one never scans on its own, `/scan` is a separate, explicit step:
   ...
   ```
   then issue `/duration <args>` — or run it non-interactively if the user gave concrete arguments.
