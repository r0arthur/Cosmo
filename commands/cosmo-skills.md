---
description: cosmo /skills — [add <path>] — list skills matched to the target, or load one ad hoc
allowed-tools: Bash(cosmo:*), Bash(python -m cosmo:*)
---

Run the cosmo `/skills` command in a live session over the current target and report the result.
...
1. Open a session on the target (current working tree, or a PR the user named):
   ...
   ```
   then issue `/skills <args>` — or run it non-interactively if the user gave concrete arguments.
