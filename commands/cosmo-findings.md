---
description: cosmo /findings — browse every finding from the last scan, most severe first
allowed-tools: Bash(cosmo:*), Bash(python -m cosmo:*)
---

Run the cosmo `/findings` command in a live session over the current target and report the result.
...
1. Open a session on the target (current working tree, or a PR the user named) — opening one never scans on its own, `/scan` is a separate, explicit step:
   ...
   ```
   then issue `/findings <args>` — or run it non-interactively if the user gave concrete arguments.
