...
Run the cosmo `/disclose` command in a live session over the current target and report the result.
...
1. Open a session on the target (current working tree, or a PR the user named):
   ...
   ```
   then issue `/disclose <args>` — or run it non-interactively if the user gave concrete arguments.
