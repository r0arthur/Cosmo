---
description: cosmo /waive — <finding-id> [reason] — waive a finding into the baseline
allowed-tools: Bash(cosmo:*), Bash(python -m cosmo:*)
---

Run the cosmo `/waive` command in a live session over the current target and report the result.
...
1. Open a session on the target (current working tree, or a PR the user named):
   ...
   ```
   then issue `/waive <args>` — or run it non-interactively if the user gave concrete arguments.
