---
description: cosmo /tools — static scanners: installed, version, and what a missing one costs
allowed-tools: Bash(cosmo:*), Bash(python -m cosmo:*)
---

Run the cosmo `/tools` command in a live session over the current target and report the result.
...
1. Open a session on the target (current working tree, or a PR the user named):
   ...
   ```
   then issue `/tools <args>` — or run it non-interactively if the user gave concrete arguments.
