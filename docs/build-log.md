# Build log — the 20-step implementation

cosmo was built as a step-by-step implementation of a design document
([`architecture.html`](architecture.html)), each step delivering a capability and
carrying a stated safety property. This is that record, preserved from the
original README.

For how the pieces fit together today, see **[architecture.md](architecture.md)**.
For how to use them, **[usage.md](usage.md)**.

---

**Step 7 — the egress broker — is present** (`cosmo.broker`), built ahead
...
**Step 8 — the dynamic analysis sandbox — is present** (`cosmo.sandbox`),
...
**Step 9 — the multi-model provider layer — is present** (`cosmo.providers`),
...
**Step 10 — the skills system — is present** (`cosmo.skills`). Org- and
...
**Step 11 — context ingestion — is present** (`cosmo.context`). Reads
...
posts. Wired into `run_review` for GitHub targets. See `tests/test_context.py`.

**Step 12 — incremental scanning — is present** (`cosmo.cache`). Caches
...
**Step 13 — the git hook adapter — is present** (`cosmo.triggers`). A thin
caller of `run_review` over the **staged** diff (`git diff --cached`), with the
 defaults: `critical` threshold, short output, opt-in blocking, static + LLM
...
**Step 14 — the GitHub Action adapter — is present** (`cosmo.triggers`). A
...
**Step 15 — the trend + findings store and compliance mapping — is
...
fraction), which is the signal that feeds the skills feedback loop, and holds
the coordinated-disclosure queue. The compliance layer rolls findings up to
...
**Step 16 — the zero-day fuzzing campaign — is present** (`cosmo.fuzz`), a
...
- **Hard scope constraint** — no code path accepts an external URL as a target;
  ...
  egress is stamped `SANDBOX` mode through the broker.
...
**Step 17 — the interactive command layer — is present** (`cosmo.interactive`).
...
`/scope` (step 19) and `/disclose` (step 18) register into this same
...
**Step 18 — the coordinated disclosure workflow — is present**
...
**Step 19 — authorized external-target mode — is present** (`cosmo.external`).
Testing a live target the user is *separately authorized* to test (a bug-bounty
program, an approved pentest) — distinct from sandbox confirmation and
fuzzing, which only ever touch a build cosmo provisioned itself, with
**no code path between the two**. All enforcement
already lives in the egress broker (step 7); this layer adds `/scope`
declaration parsing and a thin recon driver. The guarantees hold under test:
...
interactive session, completing the command set. See `tests/test_external.py`.

**Step 20 — Claude Code plugin/skill integration — is present**
...
Code to be installed (reuse, not fork). The load-bearing property is that the
...
- **Every command is tool-narrowed.** Each generated command file restricts
  ...
  a `gh … comment` posting tool. Public posting stays behind the gate *inside*
...
- **The in-process bridge routes through the same `dispatch`.** `PluginBridge`
  ...
  config as the ceiling and cannot inject a safety-tier value.
...
| 7 | Egress broker — single chokepoint, mode gate + scope + rate + log (RISK-02) | real, tested |
| 8 | Dynamic sandbox — provision/health/confirm/evidence/teardown | real, tested |
| 9 | Multi-model provider layer — alternates, resolution order, ensemble, gate | real, tested |
...
| 12 | Incremental scanning — per-file / composite-key caching | real, tested |
| 13 | Git-hook adapter (pre-commit / pre-push) | real, tested |
| 14 | GitHub Action adapter — gated PR comment (RISK-05) | real, tested |
| 15 | Trend store + CWE→OWASP compliance mapping | real, tested |
| 16 | Zero-day fuzzing — harness gen, engines, triage, novelty-never-asserted | real, tested |
| 17 | Interactive command layer — `/…` over a live session, no guardrail bypass | real, tested |
| 18 | Coordinated disclosure — drafts-and-**queues**, human approval required (RISK-06) | real, tested |
| 19 | Authorized external-target mode — `/scope`, exclusions, logging | real, tested |
| 20 | Claude Code plugin/skill integration — generated surface, no bypass | real, tested |
